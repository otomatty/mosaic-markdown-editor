...
// プリセットテーマからMUIテーマを作成
export const createPresetTheme = (presetTheme: PresetTheme): Theme => {
  const colors = presetThemeColors[presetTheme]
  const isDark = presetTheme.includes('dark') || presetTheme === 'one-dark' || presetTheme === 'monokai'
...
export const createThemeFromPreset = (presetTheme: PresetTheme): Theme => {
  return createPresetTheme(presetTheme)
}
